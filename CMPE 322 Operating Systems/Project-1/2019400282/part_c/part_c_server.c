 ...
 * 
 * This program is the implementation of the Server of the Part C of the CmpE 322 Project 1 in the 
 * Fall 2021 semester.
 ...
 * 
 * The client for the logger is created in the part_c_svc.c file since main code for the server is there 
 * and the arguments for part_c_server.out are taken from there. So, refer to that file. Here, we just defined
 * the values of the number 1, number 2 and the result for logging.
 * 
 * (This code is first generated by rpcgen and then modified by me. I could not fully understand some
 * techincal details generated by rpcgen, so some explanations may be missed.)
 * 
 ...
 */
part_c_result *
part_c_1_svc(part_c_struct *argp, struct svc_req *rqstp)
{
	/**
	...
	memset(&result, '\0', sizeof(part_c_result));                                
	...
	// num1_log, num2_log and result_log are global variables defined for logging. Once we 
	// have the values of num1 and num2, we set num1_log and num2_log.
	strcpy(num1_log,str_num1);
	strcpy(num2_log,str_num2);
	...
	if(pid > 0){
		...
		if(read(pipe2[READ_END], out_msg, INT_BUFFER_SIZE)){
			...
			/**
			 * If the result is successfully calculated, it is saved to the result_log variable for logging.
			 */
			strcpy(result_log, out_msg);
		}

		else if(read(pipe3[READ_END], err_msg, 10000)){
			/**
			...
			/**
			 * If the result cannot be calculated, we just log an _ (underscore) instead of the result.
			 */
			char no_log[INT_BUFFER_SIZE] = "_\n";
			strcpy(result_log, no_log);
		}
