...
/* What we need is the cross product of Restaurant, Row of Gourmet with ID 2 of 
Gourmet table and Destination table. So we LEFT JOINed them: */
LEFT JOIN Gourmet ON (Gourmet.Gourmet_ID=2)
LEFT JOIN Destination ON (Restaurant.Country_Region = Destination.Destination_ID)

/* Since there is only one row in the Gourmet table which is of Gourmet with ID 2 in 
...
(Restaurant.Average_Price <= Gourmet.Average_Budget OR
